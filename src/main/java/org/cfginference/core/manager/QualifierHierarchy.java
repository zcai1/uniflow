...
public interface QualifierHierarchy {
    ...
    /**
     * Returns the width of this hierarchy, i.e. the expected number of Qualifiers on any valid
     * type.
     *
     * @return the width of this QualifierHierarchy
     */
    default int getWidth() {
        return getTopQualifiers().size();
    }
     ...
     */
    Set<? extends Qualifier> getTopQualifiers();
     ...
     */
    Qualifier getTopQualifier(Qualifier qualifier);
     ...
     */
    Set<? extends Qualifier> getBottomQualifiers();
     ...
     */
    Qualifier getBottomQualifier(Qualifier qualifier);
     ...
     *     null} if there is no polymorphic qualifier in that hierarchy
     */
    @Nullable Qualifier getPolymorphicQualifier(Qualifier qualifier);
    ...
    default boolean isSubtype(
            Collection<? extends Qualifier> subQualifiers,
            Collection<? extends Qualifier> superQualifiers) {
        assert subQualifiers.size() == superQualifiers.size();
        for (Qualifier subQual : subQualifiers) {
            Qualifier superQual = findQualifierInSameHierarchy(superQualifiers, subQual);
            if (superQual == null) {
                throw new BugInCF(
                        "QualifierHierarchy: missing Qualifier in hierarchy %s. found: %s",
    ...
    @Nullable Qualifier leastUpperBound(
            Qualifier qualifier1, Qualifier qualifier2);
     ...
     */
    default Set<? extends Qualifier> leastUpperBounds(
            Collection<? extends Qualifier> qualifiers1,
            Collection<? extends Qualifier> qualifiers2) {
        ...
        for (Qualifier a1 : qualifiers1) {
            for (Qualifier a2 : qualifiers2) {
                Qualifier lub = leastUpperBound(a1, a2);
                if (lub != null) {
                    result.add(lub);
                }
            }
     ...
     */
    default Qualifier widenedUpperBound(
            Qualifier newQualifier, Qualifier previousQualifier) {
        Qualifier widenUpperBound = leastUpperBound(newQualifier, previousQualifier);
     ...
     * @return greatest lower bound of the two Qualifiers or null if the two Qualifiers are not
     *     from the same hierarchy
     */
    // The fact that null is returned if the qualifiers are not in the same hierarchy is used by the
    // collection version of LUB below.
    @Nullable Qualifier greatestLowerBound(
            Qualifier qualifier1, Qualifier qualifier2);
     ...
     */
    default Set<? extends Qualifier> greatestLowerBounds(
            Collection<? extends Qualifier> qualifiers1,
            Collection<? extends Qualifier> qualifiers2) {
        assertSameSize(qualifiers1, qualifiers2);
        if (qualifiers1.isEmpty()) {
        ...
        for (Qualifier a1 : qualifiers1) {
            for (Qualifier a2 : qualifiers2) {
                Qualifier glb = greatestLowerBound(a1, a2);
                if (glb != null) {
                    result.add(glb);
                }
            }
    ...
    /**
     * Returns true if and only if {@link QualifiedType#getQualifiers()} can return a set
     * with fewer qualifiers than the width of the QualifierHierarchy.
     *
     * @param type the type to test
     * @return true if and only if {@link QualifiedType#getQualifiers()} can return a set
     *     with fewer qualifiers than the width of the QualifierHierarchy
     */
    static boolean canHaveEmptyQualifierSet(QualifiedType type) {
        TypeKind kind = type.getJavaType().getKind();
    ...
    /**
     * Returns the Qualifier in {@code qualifiers} that is in the same hierarchy as {@code
     * qualifier}.
     *
     * <p>The default implementation calls {@link #getTopQualifier(Qualifier)} and then
     * calls {@link #findQualifierInHierarchy(Collection, Qualifier)}. So, if {@code
     * qualifier} is a top qualifier, then call {@link #findQualifierInHierarchy(Collection,
     * Qualifier)} directly is faster.
     *
     * @param qualifiers set of Qualifiers to search
     * @param qualifier Qualifier that is in the same hierarchy as the returned Qualifier
     * @return Qualifier in the same hierarchy as qualifier, or null if one is not found
     */
    default @Nullable Qualifier findQualifierInSameHierarchy(
            Collection<? extends Qualifier> qualifiers, Qualifier qualifier) {
        Qualifier top = this.getTopQualifier(qualifier);
        return findQualifierInHierarchy(qualifiers, top);
    }

    /**
     * Returns the Qualifier in {@code qualifiers} that is in the hierarchy for which {@code top}
     * is top.
     *
     * @param qualifiers set of Qualifiers to search
     * @param top the top Qualifier in the hierarchy to which the returned Qualifier belongs
     * @return Qualifier in the same hierarchy as Qualifier, or null if one is not found
     */
    default @Nullable Qualifier findQualifierInHierarchy(
            Collection<? extends Qualifier> qualifiers, Qualifier top) {
        for (Qualifier anno : qualifiers) {
    ...
    /**
     * Update a mapping from {@code key} to a set of Qualifiers. If {@code key} is not
    ...
    default <T> boolean updateMappingToMutableSet(
            Map<T, Set<Qualifier>> map, T key, Qualifier qualifier) {
        ...
        if (mapContainsKey) {
            ...
            @NonNull Set<Qualifier> prevs = map.get(key);
            Qualifier old = findQualifierInSameHierarchy(prevs, qualifier);
            if (old != null) {
                return false;
            }
            prevs.add(qualifier);
            ...
            Set<Qualifier> set = new LinkedHashSet<>();
            set.add(qualifier);
            map.put(key, set);
        }
        return true;
